...
test.describe("multivendor storefront smoke (offline-tolerant)", () => {
  test("home renders", async ({ page }) => {
    await page.goto("/en");
    await expect(page.getByRole("heading", { name: "Home", level: 1 })).toBeVisible();
  });

  test("become a vendor page reachable", async ({ page }) => {
    await page.goto("/en/become-a-vendor");
    await expect(page.getByRole("heading", { name: "Become a Vendor", level: 1 })).toBeVisible();
  });
