/**
 * P0 core commerce E2E: catalog, PDP, cart (MV API + seed). Run via `yarn test:e2e:core` first.
...
test.describe("catalog + cart (live API, MV)", () => {
  ...
  test.beforeEach(() => {
    test.skip(
      !catalogTestsEnabled,
      `Backend not reachable at ${apiOrigin} — start MV backend or set PLAYWRIGHT_API_ORIGIN`,
    );
  });
  ...
  test("vendors directory loads", async ({ page }) => {
