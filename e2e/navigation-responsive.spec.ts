/** P1 — header vs mobile drawer (MV). See docs/frontend/TESTING.md */
import { test, expect } from "@playwright/test";

test.describe("responsive navigation (desktop, MV)", () => {
  ...
  test("toolbar cart is the only cart link in header; hamburger hidden", async ({ page }) => {
    ...
    await expect(
      page.getByRole("navigation", { name: "Primary navigation" }).getByRole("link", { name: "Vendors" }),
    ).toBeVisible();
  });
});

test.describe("responsive navigation (mobile, MV)", () => {
  ...
  test("drawer holds shop links and cart; primary horizontal nav hidden", async ({ page }) => {
    ...
    await expect(mobileNav.getByRole("link", { name: "Vendors" })).toBeVisible();
