        ...
        {slides.map((s, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="absolute inset-0 bg-linear-to-b from-background/85 via-background/70 to-background/90"
                aria-hidden
              />
              <div className="relative z-10 flex min-h-[280px] flex-col justify-center px-10 py-10 sm:min-h-[320px] sm:px-14 sm:py-12 md:px-16">
                <div className="mx-auto max-w-3xl text-center">
                  ...
                  {s.subheading ? (
                    <p className="mt-3 text-base text-muted-foreground sm:text-lg">{s.subheading}</p>
                  ) : null}
                  {s.ctaLabel && s.ctaUrl ? (
                    <div className="mt-6">
                      <Link
        ...
        {count > 1 ? (
          <>
            <button
              type="button"
              onClick={() => go(index - 1)}
              className="absolute left-2 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-border/80 bg-background/85 text-foreground shadow-sm backdrop-blur-sm transition hover:bg-background sm:left-3 sm:h-11 sm:w-11"
            ...
            <button
              type="button"
              onClick={() => go(index + 1)}
              className="absolute right-2 top-1/2 z-20 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-border/80 bg-background/85 text-foreground shadow-sm backdrop-blur-sm transition hover:bg-background sm:right-3 sm:h-11 sm:w-11"
              ...
              {slides.map((s, i) => (
                <button
                  ...
                  className={`h-2 rounded-full transition ${
                    i === index ? "w-6 bg-primary" : "w-2 bg-muted-foreground/50 hover:bg-muted-foreground/70"
                  }`}
                />
              ))}
            </div>
